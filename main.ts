  ...
  async onload() {
    ...
    if (this.settings.enableAPIServer) {
      try {
        ...
        console.log(`API Server started on ${this.settings.apiServerHost}:${this.settings.apiServerPort}`);
      } catch (error) {
...
5. 请直接开始回答，不要说“基于以上信息”之类的话。\n`;
  ...
  async loadSettings() {
    ...
    console.log('[Settings] Raw data savedWritingStyles:', raw?.savedWritingStyles);
    console.log('[Settings] Raw data connections:', raw?.connections?.length, raw?.connections?.map((c: any) => c?.name));
    this.settings = Object.assign({}, DEFAULT_SETTINGS, raw);
    console.log('[Settings] After merge savedWritingStyles:', this.settings.savedWritingStyles);
    console.log('[Settings] After merge connections:', this.settings.connections?.length, this.settings.connections?.map(c => c?.name));
  ...
  async saveSettings() {
    ...
    console.log('[Settings] Saving, savedWritingStyles:', this.settings.savedWritingStyles);
    console.log('[Settings] savedWritingStyles is Array:', Array.isArray(this.settings.savedWritingStyles));
    console.log('[Settings] Saving, connections:', this.settings.connections?.length, this.settings.connections?.map(c => c?.name));
    console.log('[Settings] Full settings keys:', Object.keys(this.settings));
    ...
    try {
      ...
      console.log('[Settings] Verify after save, savedWritingStyles:', verify?.savedWritingStyles);
      console.log('[Settings] Verify after save, connections:', verify?.connections?.length, verify?.connections?.map((c: any) => c?.name));
    } catch (e) {
  ...
  private initAutoIndexScheduler() {
    ...
    if (!this.settings.autoIndexEnabled) {
      console.log('[AutoIndex] Auto index is disabled');
      return;
    }
    ...
    if (intervalMinutes > 0) {
      ...
      console.log(`[AutoIndex] Setting up interval update every ${intervalMinutes} minutes`);
    ...
    console.log('[AutoIndex] Auto index scheduler initialized');
  }
  ...
  private scheduleNextDailyIndex(time: string) {
    ...
    console.log(`[AutoIndex] Next scheduled index at ${target.toLocaleString()} (in ${Math.round(delay / 60000)} minutes)`);
  ...
  private async runAutoIndex(trigger: 'interval' | 'scheduled') {
    ...
    if (now - this.lastAutoIndexTime < 5 * 60 * 1000) {
      console.log('[AutoIndex] Skipped: too soon since last index');
    ...
    if (!this.settings.autoIndexEnabled) {
      console.log('[AutoIndex] Skipped: auto index disabled');
      this.settings.lastAutoIndexRunAt = now;
      this.settings.lastAutoIndexStatus = 'skipped';
      this.settings.lastAutoIndexTrigger = trigger;
    ...
    console.log(`[AutoIndex] Running auto index (trigger: ${trigger})`);
