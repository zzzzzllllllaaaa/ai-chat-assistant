...
export class AiChatAssistantSettingTab extends PluginSettingTab {
  ...
  private renderSkillsSection(containerEl: HTMLElement) {
    ...
    // 内置技能管理
    new Setting(skillsSectionEl).setName("内置技能").setHeading();
    ...
    // 社区技能源
    new Setting(skillsSectionEl).setName("社区技能源").setHeading();
    ...
    // 远程技能服务
    new Setting(skillsSectionEl).setName("远程技能服务").setHeading();
    skillsSectionEl.createEl("p", { 
  ...
  private renderMcpSection(containerEl: HTMLElement) {
    ...
    if (this.plugin.settings.enableDashScopeMcp) {
      ...
      if (services.length > 0) {
        ...
        serviceListContainer.setCssStyles({ marginTop: "8px" });
        
        for (const svc of services) {
          const svcEl = serviceListContainer.createDiv({ cls: "mcp-service-item-compact" });
          svcEl.setCssStyles({ padding: "8px 12px" });
          svcEl.setCssStyles({ marginBottom: "4px" });
          svcEl.setCssStyles({ background: "var(--background-secondary)" });
          svcEl.setCssStyles({ borderRadius: "4px" });
          svcEl.setCssStyles({ display: "flex" });
          svcEl.setCssStyles({ justifyContent: "space-between" });
          svcEl.setCssStyles({ alignItems: "center" });
          
          const nameEl = svcEl.createSpan({ text: svc.name });
          nameEl.setCssStyles({ fontWeight: "500" });
          ...
          countEl.setCssStyles({ fontSize: "12px" });
        }
      } else {
  ...
  private renderGenericMcpSection(parentEl: HTMLElement) {
    ...
    sectionEl.setCssStyles({ marginTop: "16px" });
    sectionEl.setCssStyles({ borderTop: "1px solid var(--background-modifier-border)" });
    sectionEl.setCssStyles({ paddingTop: "12px" });
    ...
    headerEl.setCssStyles({ marginBottom: "8px" });
    
    new Setting(headerEl).setName("通用 MCP 服务器").setHeading();

    const descEl = sectionEl.createEl("p", { 
      text: "连接第三方 MCP Server（如 Agenda、自建服务等）。支持 SSE 和 Streamable HTTP 传输。", 
    });
    descEl.setCssStyles({ color: "var(--text-muted)" });
    descEl.setCssStyles({ fontSize: "12px" });
    descEl.setCssStyles({ marginTop: "0" });
    descEl.setCssStyles({ marginBottom: "12px" });
    ...
    if (servers.length > 0) {
      ...
      for (const server of servers) {
        ...
        itemEl.setCssStyles({ padding: "10px 12px" });
        itemEl.setCssStyles({ marginBottom: "6px" });
        itemEl.setCssStyles({ background: "var(--background-secondary)" });
        itemEl.setCssStyles({ borderRadius: "6px" });
        itemEl.setCssStyles({ border: "1px solid var(--background-modifier-border)" });
        ...
        topRow.setCssStyles({ marginBottom: "4px" });

        const nameEl = topRow.createSpan({ text: server.name });
        nameEl.setCssStyles({ fontWeight: "600" });
        nameEl.setCssStyles({ fontSize: "14px" });
        ...
        statusEl.setCssStyles({ fontSize: "12px" });

        // 中间：endpoint + 工具数
        const infoEl = itemEl.createDiv();
        infoEl.setCssStyles({ fontSize: "12px" });
        infoEl.setCssStyles({ color: "var(--text-muted)" });
        infoEl.setCssStyles({ marginBottom: "6px" });
        ...
        if (server.lastError) {
          ...
          errEl.setCssStyles({ fontSize: "11px" });
          errEl.setCssStyles({ color: "var(--text-error)" });
          errEl.setCssStyles({ marginBottom: "6px" });
          errEl.textContent = `错误: ${server.lastError.substring(0, 100)}`;
        }

        // 底部：操作按钮
        const actionsEl = itemEl.createDiv();
        actionsEl.setCssStyles({ display: "flex" });
        actionsEl.setCssStyles({ gap: "6px" });
        actionsEl.setCssStyles({ flexWrap: "wrap" });
        ...
        toggleBtn.setCssStyles({ fontSize: "12px" });
        toggleBtn.setCssStyles({ padding: "2px 8px" });
        ...
        refreshBtn.setCssStyles({ fontSize: "12px" });
        refreshBtn.setCssStyles({ padding: "2px 8px" });
        ...
        editBtn.setCssStyles({ fontSize: "12px" });
        editBtn.setCssStyles({ padding: "2px 8px" });
        ...
        deleteBtn.setCssStyles({ fontSize: "12px" });
        deleteBtn.setCssStyles({ padding: "2px 8px" });
      ...
      emptyEl.setCssStyles({ color: "var(--text-muted)" });
      emptyEl.setCssStyles({ fontSize: "12px" });
      emptyEl.setCssStyles({ fontStyle: "italic" });
    }
  }
  ...
  private openGenericMcpServerEditor(
    ...
    modal.contentEl.setCssStyles({ minWidth: "400px" });
    ...
    btnContainer.setCssStyles({ gap: "8px" });
    btnContainer.setCssStyles({ marginTop: "16px" });
...
class McpServiceDetailModal extends Modal {
  ...
  onOpen() {
    ...
    endpointCode.setCssStyles({ display: "block" });
    endpointCode.setCssStyles({ padding: "8px" });
    endpointCode.setCssStyles({ background: "var(--background-secondary)" });
    endpointCode.setCssStyles({ borderRadius: "4px" });
    endpointCode.setCssStyles({ wordBreak: "break-all" });
    ...
    if (this.service.tools && this.service.tools.length > 0) {
      ...
      toolsSection.createEl("p", { 
        ...
        cls: "text-muted"
      }).setCssStyles({ fontSize: "12px" });
      
      for (const tool of this.service.tools) {
        const toolEl = toolsSection.createDiv({ cls: "mcp-tool-item" });
        toolEl.setCssStyles({ padding: "8px" });
        toolEl.setCssStyles({ marginBottom: "8px" });
        toolEl.setCssStyles({ background: "var(--background-secondary)" });
        toolEl.setCssStyles({ borderRadius: "4px" });
        
        const nameEl = toolEl.createEl("strong", { text: tool.name });
        nameEl.setCssStyles({ fontFamily: "var(--font-monospace)" });
        nameEl.setCssStyles({ color: "var(--text-accent)" });
        if (tool.description) {
        ...
        if (tool.inputSchema) {
          ...
          pre.setCssStyles({ maxHeight: "200px" });
          pre.createEl("code", { 
