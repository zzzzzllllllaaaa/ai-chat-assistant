...
export class AiChatView extends ItemView implements IAiChatView {
  ...
  private addContextItem(item: ContextItem) {
   ...
   private renderContextItems() {
     ...
     this.contextContainer.style.display = "flex";
  ...
  private async handleUserMessage(message: string, images: string[] = []) {
    ...
    console.log('[AgentV2] @skill 检测:', { message: message.slice(0, 50), mentionedSkillId });
    if (mentionedSkillId) {
      try {
        ...
        // 调试: 输出 agent 结果
        console.log('[AgentV2] 技能:', skill?.name, '类型:', 
          this.agentV2['router']?.detect?.(message)?.type, 
          '结果:', agentResult.success, 
          '内容长度:', agentResult.content?.length || 0,
          '工具调用:', agentResult.toolCalls?.length || 0
        );
    ...
    try {
      ...
      errorEl.innerHTML = `<div class="ai-chat-error-content">${errorMsg}</div>`;
  ...
  private editMessage(messageEl: HTMLElement, index: number) {
    ...
    const cleanup = () => {
      ...
      (contentDiv as HTMLElement).style.display = '';
