import { App, MarkdownRenderer, setIcon, Notice, MarkdownView } from "obsidian";
...
export class MessageRenderer {
    ...
    public async renderMessage(message: ChatMessage, index: number, isStreaming: boolean = false) {
        ...
        if (message.role === 'assistant') {
            ...
            await MarkdownRenderer.render(this.app, message.content || '', contentDiv, '', this.plugin);
        }
    ...
    private async renderThoughtBlock(container: HTMLElement, content: string) {
        ...
        await MarkdownRenderer.render(this.app, content, body, '', this.plugin);
    ...
    private renderPlanBlock(container: HTMLElement, jsonContent: string) {
        try {
            ...
            if (plan.goal) {
                ...
                goalDiv.innerHTML = `<strong>目标：</strong>${this.escapeHtml(plan.goal)}`;
            }
    ...
    private async renderTextBlock(container: HTMLElement, content: string, references?: any[]) {
        ...
        await MarkdownRenderer.render(this.app, processedContent, textDiv, '', this.plugin);
