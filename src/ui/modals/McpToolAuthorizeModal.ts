...
export class McpToolAuthorizeModal extends Modal {
  ...
  onOpen() {
    ...
    this.statusEl.setCssStyles({ marginBottom: "8px" });
    this.setStatus("请选择操作。")

    const controls = contentEl.createDiv();
    controls.setCssStyles({ display: "flex" });
    controls.setCssStyles({ gap: "8px" });
    controls.setCssStyles({ marginBottom: "12px" });
    ...
    this.listEl.setCssStyles({ maxHeight: "420px" });
    this.listEl.setCssStyles({ overflowY: "auto" });
    this.listEl.setCssStyles({ border: "1px solid var(--background-modifier-border)" });
    this.listEl.setCssStyles({ borderRadius: "6px" });
    this.listEl.setCssStyles({ padding: "8px" });
