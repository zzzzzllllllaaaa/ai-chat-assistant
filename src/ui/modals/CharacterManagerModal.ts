...
export class CharacterManagerModal extends Modal {
        ...
        try {
          ...
          if (result.success && result.data) {
            ...
            if (character.characterBook && character.characterBook.entries && character.characterBook.entries.length > 0) {
              console.log(`[CharacterImport] 检测到角色知识库，共 ${character.characterBook.entries.length} 条目`);
              ...
              console.log(`[CharacterImport] 角色 "${character.name}" 没有知识库`);
            }
            ...
            if (result.warnings && result.warnings.length > 0) {
              console.log(`[CharacterImport] ${file.name} 警告:`, result.warnings);
            }
          } else {
  ...
  private async saveCharacterBookAsMemory(
    ...
    try {
      ...
      console.log(`[CharacterImport] 角色知识库已保存: ${lorebookPath}，共 ${entries.length} 条（启用 ${enabledCount}，禁用 ${disabledCount}）`);
    ...
    if (persona.avatar) {
      if (persona.avatar.startsWith('data:image/png')) {
        ...
        try {
          ...
          console.log('[ExportPNG] 无法读取头像文件:', e);
        }
      }
    }
...
class CharacterEditModal extends Modal {
  ...
  updateStyleSectionVisibility(container: HTMLElement, enabled: boolean) {
    ...
    if (detailsContainer) {
      detailsContainer.style.display = enabled ? 'block' : 'none';
    }
  }
