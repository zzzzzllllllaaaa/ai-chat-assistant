...
export class DashScopeMcpClient {
  ...
  async callToolWithMcpProtocol(toolName: string, toolArgs: any, retryCount = 0): Promise<any> {
    ...
    try {
      ...
      console.log(`[MCP] 使用标准协议调用工具: ${toolName}，端点: ${sseUrl}${retryCount > 0 ? ` (重试 ${retryCount}/${maxRetries})` : ''}`);

      // 如果是重试，先等待一段时间（更长的指数退避：5s, 15s, 30s）
      if (retryCount > 0) {
        // 使用更激进的退避策略：5秒、15秒、30秒
        const backoffMs = [5000, 15000, 30000][retryCount - 1] || 30000;
        console.log(`[MCP] 遇到 429 限流，等待 ${backoffMs / 1000}秒 后重试 (${retryCount}/${maxRetries})...`);
      ...
      while (!controller.signal.aborted) {
        ...
        if (messageEndpoint && !initSent) {
          ...
          console.log(`[MCP] 发送 initialize 请求`);
          requestUrl({
            url: messageEndpoint,
            method: "POST",
        ...
        if (initCompleted && !callSent) {
          ...
          console.log(`[MCP] 发送 tools/call 请求:`, { name: toolName, arguments: toolArgs });
          requestUrl({
            url: messageEndpoint!,
            method: "POST",
            headers: { "Authorization": authHeader, "Content-Type": "application/json" },
