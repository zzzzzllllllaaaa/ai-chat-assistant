...
export class ToolRegistry {
  ...
  private async fetchDashScopeDirectory(): Promise<McpService[]> {
    ...
    // 调试：打印原始响应以便查看结构
    console.log("[MCP] 目录 API 原始响应:", JSON.stringify(list.slice(0, 2), null, 2));

    // 将 API 返回的数据转换为 McpService 结构
    const services = list
      .map((item: any): McpService | null => {
        // 调试：打印每个 item 的关键字段
        console.log("[MCP] 服务 item:", {
          id: item?.id,
          name: item?.name,
          toolName: item?.toolName,
          operationalUrl: item?.operationalUrl,
        });
  ...
  private async fetchServiceTools(service: McpService, retryCount = 0): Promise<McpToolDefinition[]> {
    ...
    try {
      ...
      if (retryCount > 0) {
        ...
        console.log(`[MCP] 服务 ${service.id} 遇到 429 限流，等待 ${backoffMs / 1000}秒 后重试 (${retryCount}/${maxRetries})...`);
        await new Promise(r => setTimeout(r, backoffMs));
      }

      console.log(`[MCP] 获取服务 ${service.id} 的工具列表，端点: ${service.endpoint}${retryCount > 0 ? ` (重试 ${retryCount})` : ''}`);
      ...
      const processLines = (lines: string[]): boolean => {
        ...
        for (const line of lines) {
          ...
          if (line.startsWith("data:")) {
            ...
            console.log(`[MCP] SSE 事件 [${currentEventType || 'message'}]:`, data.slice(0, 300));
            ...
            if (currentEventType === "endpoint" || (!messageEndpoint && (data.includes("sessionId") || data.includes("/message")))) {
              ...
              if (url.startsWith("/") || url.startsWith("http")) {
                try {
                  ...
                  console.log(`[MCP] 获取到消息端点: ${messageEndpoint}`);
                } catch (e) {
            ...
            try {
              ...
              if (json?.result?.protocolVersion || json?.result?.serverInfo) {
                console.log(`[MCP] 初始化成功:`, json.result);
                initCompleted = true;
                continue;
              }
              ...
              if (json?.result?.tools && Array.isArray(json.result.tools)) {
                ...
                console.log(`[MCP] 收到工具列表响应，${tools.length} 个工具`);
                toolsReceived = true;
                return true; // 停止解析
              }
      ...
      while (!controller.signal.aborted && !toolsReceived) {
        ...
        if (messageEndpoint && !initSent) {
          ...
          console.log(`[MCP] 发送 initialize 请求到: ${messageEndpoint}`);
        ...
        if (initCompleted && !listSent) {
          ...
          console.log(`[MCP] 发送 tools/list 请求到: ${messageEndpoint}`);
  ...
  async refresh(provider: McpProvider): Promise<void> {
    ...
    for (const svc of services) {
      try {
        ...
        if (serviceIndex > 0) {
          console.log(`[MCP] 等待 3 秒后获取下一个服务...`);
          await new Promise(r => setTimeout(r, 3000));
        }
        serviceIndex++;
        
        console.log(`[MCP] 开始获取服务 ${svc.id} 的工具列表 (${serviceIndex}/${services.length})...`);
        const tools = await this.fetchServiceTools(svc);
        if (tools.length > 0) {
          svc.tools = tools;
          console.log(`[MCP] ✅ 服务 ${svc.id} 获取到 ${tools.length} 个工具:`, tools.map(t => t.name));
        } else {
          ...
          console.log(`[MCP] ⚠️ 服务 ${svc.id} 未获取到工具，使用默认工具名:`, fallbackToolName);
        }
      } catch (e) {
        // 忽略单个服务的工具获取失败
        const fallbackToolName = `bailian_${camelToSnake(svc.id)}`;
  ...
  async refreshGenericServer(serverId: string): Promise<{ tools: McpToolDefinition[]; error?: string }> {
    ...
    try {
      ...
      console.log(`[MCP] ✅ 通用服务器 ${server.name} 获取到 ${toolDefs.length} 个工具:`, toolDefs.map(t => t.name));
